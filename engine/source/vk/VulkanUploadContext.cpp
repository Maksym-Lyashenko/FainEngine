...
void VulkanUploadContext::uploadImage2D(
    VulkanAllocator* allocator,
    const void* data,
    size_t sizeBytes,
    VkImage dstImage,
    ...
    uint32_t height)
{
  ...
  if (data == nullptr)
  {
    throw std::runtime_error("VulkanUploadContext::uploadImage2D(): data is null");
  }

  if (dstImage == VK_NULL_HANDLE)
  {
    throw std::runtime_error("VulkanUploadContext::uploadImage2D(): destination image is invalid");
  }
